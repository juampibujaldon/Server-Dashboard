...
def test_read_metrics(db):
    ...
    docs = list(db.metrics.find({"serverId": "srv-crud"}))
    assert len(docs) >= 2

def test_update_metric(db):
    db.metrics.insert_one(Metric("srv-upd", 1.0, 2.0, 3.0, 4.0).to_dict())
    res = db.metrics.update_one({"serverId": "srv-upd"}, {"$set": {"cpu_usage": 9.9}})
    assert res.modified_count == 1
    doc = db.metrics.find_one({"serverId": "srv-upd"})
    assert doc["cpu_usage"] == 9.9

def test_delete_metric(db):
    db.metrics.insert_one(Metric("srv-del", 1.0, 2.0, 3.0, 4.0).to_dict())
    res = db.metrics.delete_one({"serverId": "srv-del"})
    assert res.deleted_count == 1
    assert db.metrics.find_one({"serverId": "srv-del"}) is None
