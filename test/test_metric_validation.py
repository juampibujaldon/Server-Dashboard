def test_metrics_post_missing_field_returns_400(client):
    bad = {"serverId":"s","cpu_usage":1.0,"ram_usage":2.0,"disk_space":3.0}  # falta temperature
...
def test_metrics_post_out_of_range_returns_400(client):
    bad = {"serverId":"s","cpu_usage":150.0,"ram_usage":2.0,"disk_space":3.0,"temperature":10.0}
    r = client.post("/api/metrics", json=bad)
    assert r.status_code == 400
